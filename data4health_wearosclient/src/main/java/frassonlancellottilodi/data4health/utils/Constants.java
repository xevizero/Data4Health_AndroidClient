package frassonlancellottilodi.data4health.utils;

/**
 * The most important costants used throughout the application
 */
public class Constants {
    public static final String WEARABLE_DATA_PATH = "/wearable_data", PHONE_DATA_PATH = "/phone_data";
    public static final String REQUEST_CURRENT_STEPS = "REQUEST_CURRENT_STEPS";
    public static final String RESPONSE_CURRENT_STEPS = "RESPONSE_CURRENT_STEPS";
    public static final String REQUEST_SYNC_DATA_FROM_WATCH = "REQUEST_SYNC_DATA_FROM_WATCH";
    public static final String REQUEST_EMERGENCY_SOS = "REQUEST_EMERGENCY_SOS";
    public static final String FALL = "FALL";

}